        ...
        <Box style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <Box style={{ display: 'flex', width: '90%', justifyContent: 'space-between' }}>
                <Box style={{ display: 'flex', width: '100%', borderBottom: '2px solid black'}}>
                    <Box className={"mobile_main_header_text"}>갤러리</Box>
                                            ...
                                            return (
                                                <TableRow key={index}
                                                    ...
                                                    <TableCell id={"gallery_search_table_row_title_" + index} style={{ display: 'flex', padding: '10px 0px 10px 0px', cursor: 'point' }}>
                                                        <Link to={"/MGalleryDetailPage?no=" + gallery.no} className={"main_link_row"}>
                                                            {gallery.title}
                                                        </Link>
                                                    </TableCell>
                                                    <TableCell id={"gallery_search_table_row_createdAt_" + index} align="center" style={{ padding: '10px 0px 10px 0px' }}>{moment(new Date(gallery.createdAt)).format("YYYY-MM-DD")}</TableCell>
                                                </TableRow>
                                            )
                                        })
                                    }
